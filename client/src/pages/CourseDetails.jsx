...
const CourseDetails = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto p-5">
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-semibold flex items-center cursor-pointer" onClick={() => setShowStudentList(!showStudentList)}>
          Students List
          {showStudentList ? <FaChevronUp className="ml-2" /> : <FaChevronDown className="ml-2" />}
        </h2>
        {showStudentList && (
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {students.map((student, index) => {
                  ...
                  const attendancePercentage = student.attendancePercentage[courseId] || 0; // Default to 0 if not found

                  const isLowAttendance = attendancePercentage < 75;
                  return (
                    <tr key={index} className={isLowAttendance ? 'bg-red-100' : ''}>
                      <td className="border p-2 text-center">{student.rollno}</td>
                      <td className="border p-2 text-center">{student.name}</td>
                      <td className="border p-2 text-center">{attendancePercentage}%</td>
                    </tr>
                  );
                })}
              </tbody>
